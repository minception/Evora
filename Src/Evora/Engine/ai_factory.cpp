...
std::map<const char*, std::unique_ptr<ai_factory>, raw_pointer_comparison> ai_factory::get_factories()
{
	...
	res.insert({ "ScoreAI", std::move(std::make_unique<score_ai_factory>()) });
...
std::unique_ptr<::ai::ai> score_ai_factory::get(std::shared_ptr<control::game_controller> controller, int board_index)
{
	return std::move(std::make_unique<score_ai>(controller, board_index));
}
