...
std::unique_ptr<control::command> strategy_move::gen_move()
{
...
void strategy_move::calculate_parameters()
{
	...
	if (m_pattern_line_index < model::COLORS)
	{
		int empty_slots = m_model->get_pattern_line_tile_count(m_player_index, m_pattern_line_index);
		m_line_overflow = m_tile_count > empty_slots ? m_tile_count - empty_slots : 0;
		int row = (m_pattern_line_index + static_cast<int>(m_color)) % model::COLORS;
		m_wall_position = std::make_tuple(m_pattern_line_index, row);
		m_line_fill = (m_pattern_line_index + 1) / m_tile_count;
	}
	else
	{
